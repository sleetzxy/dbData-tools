...
def process_csv_files(
            ...
            try:
                with (
                    ...
                    if original_table_name in column_mapping:
                        ...
                        # 提示哪些列未映射
                        unmapped_columns = [
                            col for col in raw_headers if col not in column_map
                        ]
                        if unmapped_columns:
                            logger.warning(
                        ...
                        logger.warning(
                            f"表 {original_table_name} 没有列名映射，将保留原始列名"
                        )
                ...
                result["error_files"].append({"file": filename, "error": str(e)})
    ...
    # 汇总处理结果
    logger.info(
        f"CSV处理完成，共处理 {len(result['processed_files'])} 个有映射的文件，"
