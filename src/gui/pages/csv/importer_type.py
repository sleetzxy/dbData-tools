...
class ImportCsvTypePage(BaseToolPage):
    ...
    def setup_left_panel_content(self, parent):
        ...
        self.path_selector = PathSelector(
            parent, label_text="CSV目录路径", mode="folder"
        )
        ...
        self.csv_scrollable_frame = self.ctk.CTkScrollableFrame(
            ...
            scrollbar_button_color=self.idea_dark_colors.get(
                "scrollbar_button", "#5c5f62"
            ),
            scrollbar_button_hover_color=self.idea_dark_colors.get(
        ...
        self.import_button = PrimaryButton(
            parent, text="🚀 开始导入", command=self.start_task
        )
    ...
    def _create_csv_list_header(self):
        ...
        for i, text in enumerate(labels):
            ...
            label.grid(
                row=0, column=i, sticky="w", padx=(10 if i == 0 else 5, 5), pady=8
            )
    ...
    def _create_csv_entry_row(self, csv_path):
        ...
        try:
            ...
            value_entry = StyledEntry(row_frame, textvariable=value_var)
    ...
    def apply_config(self, config):
        ...
        try:
            ...
            if path and os.path.isdir(path):
                # 延迟执行，确保界面初始化完成
                self.root.after(
                    100, lambda: self._load_csv_and_apply_config(path, config)
                )
    ...
    def _apply_type_mapping(self, config):
        ...
        try:
            ...
            mapping_by_table = {
                m["table"]: m for m in type_mapping if isinstance(m, dict)
            }
    ...
    def validate(self):
        ...
        if not self.csv_entries:
            messagebox.showerror(
                "错误",
                "未识别到任何CSV文件，请确认数据源路径有效，并在列表中完成类型列与数据类型的选择后再试。",
            )
            return False
        ...
        for entry in self.csv_entries:
            ...
            if type_type not in ["string", "number"]:
                messagebox.showerror(
                    "错误", f"请为表 {table} 选择类型数据类型（字符串或数字）"
                )
                return False
            if not type_value:
